...
# Paste your bounding boxes from pick_coords.py below.
# """

# from PIL import Image, ImageDraw, ImageFont
# import os

# # ── PASTE YOUR BOXES HERE (from pick_coords.py / coords_log.txt) ──────────
# # Format: (x1, y1, x2, y2)  — top-left to bottom-right in original px

# GROUP_STAGE_BOX  = (0,    0,    0,    0)    # ← replace with your values
# HOME_CREST_BOX   = (0,    0,    0,    0)    # ← replace
# AWAY_CREST_BOX   = (0,    0,    0,    0)    # ← replace
# HOME_SCORE_BOX   = (294,  843,  473,  989)  # ← replace
# AWAY_SCORE_BOX   = (603,  839,  765,  987)  # ← replace
# STADIUM_BOX      = (0,    0,    0,    0)    # ← replace
# HOME_SCORERS_BOX = (112,  1076, 519,  1427) # ← replace
# AWAY_SCORERS_BOX = (563,  1073, 954,  1427) # ← replace
...
# # ── GOAL SYMBOL IMAGES ───────────────────────────────────────────────────────
# SYMBOL_GOAL           = 'assets/symbols/goal_regular.png'
# SYMBOL_PENALTY        = 'assets/symbols/goal_penalty.png'
# SYMBOL_OWN            = 'assets/symbols/goal_owngoal.png'
# SYMBOL_PENALTY_MISSED = 'assets/symbols/penalty_missed.png'  # optional

# # ── COLOURS ─────────────────────────────────────────────────────────────────
# COLOR_SCORE   = (255, 255, 255, 255)
...
#     """Return True if all scorer lines fit inside box at font_size."""
#     if not lines:
#         return True
...
#     bh     = box[3] - box[1]
#     needed = len(lines) * step - SCORER_LINE_GAP
#     return needed <= bh
...
#     """Map scraper event type string to a local symbol image path."""
#     t = event_type.lower()
#     if t == 'penalty_goal':
#         return SYMBOL_PENALTY
#     if t == 'own_goal':
#         return SYMBOL_OWN
#     if t == 'penalty_missed':
#         return SYMBOL_PENALTY_MISSED
#     return SYMBOL_GOAL


# def _extract_scorer_lines(events: list, team_name: str) -> list[dict]:
#     """
#     Pull goal events for team_name from the scraper events list.
#     Returns [{'surname': str, 'minute': str, 'symbol_path': str}, ...]
#     """
#     lines = []
#     if not isinstance(events, list):
#         return lines
#     for ev in events:
#         if ev.get('type') not in ('goal', 'penalty_goal', 'own_goal'):
#             continue
...
#             'surname':     surname,
#             'minute':      minute,
#             'symbol_path': _goal_symbol_path(ev.get('type', 'goal')),
#         })
#     return lines
...
#     # Work out how many lines fit, reserve 1 slot for "& N more" if needed
#     visible = []
#     hidden  = 0
#     for entry in lines:
#         needed_bot = y1 + len(visible) * step + lh
#         remaining  = len(lines) - len(visible) - 1
#         # If this line would push us over AND there are still more after it,
#         # reserve space for the "& N more" footer
...
#         cy       = y1 + i * step
#         text_str = f"{entry['surname']}  {entry['minute']}'"
#         tw, _    = _text_size(draw, text_str, font)
#         sym_img  = _load_symbol(entry['symbol_path'], sym_h)
#         sw       = sym_img.width if sym_img else 0

#         if align == 'right':
#             text_x = x2 - sw - (sym_gap if sw else 0) - tw
#             sym_x  = x2 - sw
#             text_x = max(text_x, x1)
#         else:
#             text_x = x1
#             sym_x  = x1 + tw + sym_gap
#             if sym_x + sw > x2:
...
#         draw.text((text_x, cy), text_str, font=font, fill=COLOR_SCORER)

#         if sym_img:
#             paste_y = cy + (lh - sym_h) // 2
#             img.paste(sym_img, (sym_x, paste_y), sym_img)

#     if hidden > 0:
#         more_str = f"& {hidden} more"
#         fy = y1 + len(visible) * step
#         mw, _  = _text_size(draw, more_str, font)
#         mx = (x2 - mw) if align == 'right' else x1
...
# def generate_scorecard(scraper_data: dict, event_type: str = 'FT') -> str:
#     """
...
#     match_id     = str(match_sample.get('matchID', 'unknown'))
...
#         # Use event_type-prefixed key so HT and FT templates are separate
#         template_key  = f"{event_type.lower()}_{match_id}"
...
#         size = _fit_font_to_box(draw, score_val, box, FONT_BOLD,
#                                 SCORE_FONT_MAX, SCORE_FONT_MIN)
#         font = _font(FONT_BOLD, size)
#         bx   = (box[0] + box[2]) // 2
#         by   = (box[1] + box[3]) // 2
#         draw.text((bx, by), score_val, font=font, fill=COLOR_SCORE, anchor='mm')

#     # ── Crests ────────────────────────────────────────────────────────────────
#     home_crest = match_sample.get('team_A_img')
#     away_crest = match_sample.get('team_B_img')
...
#     # ── Group / Stage label ───────────────────────────────────────────────────
#     if GROUP_STAGE_BOX != (0, 0, 0, 0):
#         stage_text = match_sample.get('leagueName', '')
#         if stage_text:
#             size = _fit_font_to_box(draw, stage_text, GROUP_STAGE_BOX,
#                                     FONT_BOLD, 28, 12)
#             font = _font(FONT_BOLD, size)
#             bx   = (GROUP_STAGE_BOX[0] + GROUP_STAGE_BOX[2]) // 2
#             by   = (GROUP_STAGE_BOX[1] + GROUP_STAGE_BOX[3]) // 2
#             draw.text((bx, by), stage_text, font=font,
#                       fill=COLOR_SCORE, anchor='mm')

#     # ── Stadium ───────────────────────────────────────────────────────────────
#     if STADIUM_BOX != (0, 0, 0, 0):
#         stadium = match_sample.get('stadium', '')
#         if stadium:
#             size = _fit_font_to_box(draw, stadium, STADIUM_BOX,
#                                     FONT_REGULAR, 24, 10)
#             font = _font(FONT_REGULAR, size)
#             bx   = (STADIUM_BOX[0] + STADIUM_BOX[2]) // 2
#             by   = (STADIUM_BOX[1] + STADIUM_BOX[3]) // 2
#             draw.text((bx, by), stadium, font=font,
#                       fill=COLOR_SCORER, anchor='mm')

#     # ── Scorer lines ──────────────────────────────────────────────────────────
#     home_lines = _extract_scorer_lines(events, home_team)
#     away_lines = _extract_scorer_lines(events, away_team)
...
#     _draw_scorer_lines(img, draw, home_lines, HOME_SCORERS_BOX, 'right', chosen_size)
#     _draw_scorer_lines(img, draw, away_lines, AWAY_SCORERS_BOX, 'left',  chosen_size)
